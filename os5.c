...
void* ta_actions() {
	...
	while( 1 ) {
		if ( no_students_waiting > 0 ) {
			ta_sleeping_flag = 0;
			sem_wait( &sem_students );
			pthread_mutex_lock( &mutex_thread );
			int help_time = rand() % 5;
			printf( "Helping a student for %d seconds. Students waiting = %d.\n", help_time, (no_students_waiting - 1) );
			printf( "Student %d receiving help.\n",wt_chairs[next_teaching_position] );
			wt_chairs[next_teaching_position]=0;
			no_students_waiting--;
...
void* student_actions( void* student_id ) {
	...
	while( 1 ) {
		...
		sleep( time );
		pthread_mutex_lock( &mutex_thread );
		if( no_students_waiting < NUM_WAITING_CHAIRS ) {
			wt_chairs[next_seating_position] = id_student;
			no_students_waiting++;
			printf( "\t\tStudent %d takes a seat. Students waiting = %d.\n", id_student, no_students_waiting );
...
int isWaiting( int student_id ) {
	int i;
	for ( i = 0; i < 3; i++ ) {
		if ( wt_chairs[i] == student_id ) { return 1; }
	}
	return 0;
}
